...
class FaceProcessor:
    def __init__(self, config:  Dict[str, dict], most_centered_only: bool = False):
        ...
        self.facial_attributes = self.config.get('facial_attributes', [])
        ...
        if 'age' in self.facial_attributes:
            self.models['age'] = load_model(self.config, 'age_model', self.hw_location)
        if 'gender' in self.facial_attributes:
            self.models['gender'] = load_model(self.config, 'gender_model', self.hw_location)
        if "emotion" in self.facial_attributes:
            self.models['emotion'] = load_model(self.config, 'emotion_model', self.hw_location)
    ...
    def process_face_results(self, detection_results):
        ...
        # Run batch predict on aligned faces and get the facial attributes
        # Batch Process age prediction
        if 'age' in self.models:
            age_predictions = self.models['age'].predict_batch(aligned_faces)
            for face, age in zip(face_results, age_predictions):
                face["extra_results"]["age"] = int(age.results[0]["score"])

        # Batch Process gender prediction
        if 'gender' in self.models:
            gender_predictions = self.models['gender'].predict_batch(aligned_faces)
            for face, gender in zip(face_results, gender_predictions):
                # Get the label with the maximum score
                face["extra_results"]["gender"] = max(gender.results, key=lambda x: x['score'])['label']

        # Batch Process emotion prediction
        if 'emotion' in self.models:
            emotion_predictions = self.models['emotion'].predict_batch(aligned_faces)
            for face, emotion in zip(face_results, emotion_predictions):
                face["extra_results"]["emotion"] = emotion.results[0]["label"]
